...
EL::StatusCode MyxAODAnalysis :: initialize ()
{
  ...
  vecStringGRL.push_back("$ROOTCOREBIN/data/MyAnalysis/GRL/data15_13TeV.periodAllYear_DetStatus-v73-pro19-08_DQDefects-00-01-02_PHYS_StandardGRL_All_Good_25ns.xml");
...
#undef MAKE_COUNTER_VEC
  ...
  n_passAC_twoBaseEl  = 0;
  n_passAC_twoSigEl   = 0;
  n_passAC_twoBaseMu  = 0;
  n_passAC_twoSigMu   = 0;
  n_passAC_twoBaseTau = 0;
  n_passAC_twoSigTau  = 0;
  n_passAC_threeBaseLep = 0;
  n_passAC_threeSigLep  = 0;
  n_passAC_threeBaseEl  = 0;
  n_passAC_threeSigEl   = 0;
  n_passAC_threeBaseMu  = 0;
  n_passAC_threeSigMu   = 0;
  n_passAC_oneBjet    = 0;
...
EL::StatusCode MyxAODAnalysis :: execute ()
{
  ...
  for(UInt_t eveSelec=0; eveSelec<m_vec_eveSelec->size(); eveSelec++){
    ...
    for(sysListItr = m_sysList.begin(); sysListItr != m_sysList.end(); ++sysListItr){
      ...
      myEveSelec->setMeasureMCEff(m_measureMCEff);//Need to be called before initialize();
      myEveSelec->setDoTagAndProbe(m_doTagAndProbe);//Need to be called before initialize();
...
void MyxAODAnalysis::SetEventCounterAC(EventSelector *EveSelec){
  ...
  n_passAC_twoBaseEl  += EveSelec->GetPassAC_twoBaseEl ()?1:0;
  n_passAC_twoSigEl   += EveSelec->GetPassAC_twoSigEl  ()?1:0;
  n_passAC_twoBaseMu  += EveSelec->GetPassAC_twoBaseMu ()?1:0;
  n_passAC_twoSigMu   += EveSelec->GetPassAC_twoSigMu  ()?1:0;
  n_passAC_twoBaseTau += EveSelec->GetPassAC_twoBaseTau()?1:0;
  n_passAC_twoSigTau  += EveSelec->GetPassAC_twoSigTau ()?1:0;
  n_passAC_threeBaseLep += EveSelec->GetPassAC_threeBaseLep()?1:0;
  n_passAC_threeSigLep  += EveSelec->GetPassAC_threeSigLep ()?1:0;
  n_passAC_threeBaseEl  += EveSelec->GetPassAC_threeBaseEl ()?1:0;
  n_passAC_threeSigEl   += EveSelec->GetPassAC_threeSigEl  ()?1:0;
  n_passAC_threeBaseMu  += EveSelec->GetPassAC_threeBaseMu ()?1:0;
  n_passAC_threeSigMu   += EveSelec->GetPassAC_threeSigMu  ()?1:0;
...
void MyxAODAnalysis::dumpEventCountersAC()
{
  for(UInt_t eveSelec=0; eveSelec<m_vec_eveSelec->size(); eveSelec++){
    ...
    if(eveSelecName=="ac"){
      ...
      std::cout << "  twoBaseEl :      " << n_passAC_twoBaseEl << std::endl;
      std::cout << "  twoSigEl  :      " << n_passAC_twoSigEl  << std::endl;
      std::cout << "  twoBaseMu :      " << n_passAC_twoBaseMu << std::endl;
      std::cout << "  twoSigMu  :      " << n_passAC_twoSigMu  << std::endl;
      std::cout << "  twoBaseTau:      " << n_passAC_twoBaseTau<< std::endl;
      std::cout << "  twoSigTau :      " << n_passAC_twoSigTau << std::endl;
      std::cout << "  3 BaseLep :      " << n_passAC_threeBaseLep<< std::endl;
      std::cout << "  3 SigLep  :      " << n_passAC_threeSigLep << std::endl;
      std::cout << "  3 BaseEl  :      " << n_passAC_threeBaseEl << std::endl;
      std::cout << "  3 SigEl   :      " << n_passAC_threeSigEl  << std::endl;
      std::cout << "  3 BaseMu  :      " << n_passAC_threeBaseMu << std::endl;
      std::cout << "  3 SigMu   :      " << n_passAC_threeSigMu  << std::endl;
